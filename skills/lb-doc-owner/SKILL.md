...
- 任何文档写入前，先给出逐项文档变更清单：覆盖本次扫描到的全部已有文档及拟创建文档，并将每项标为「新建 / 修改 / 不动」。已有文档未被开发者明确确认标为「修改」时，必须保持原样
...
任何写入前，协调者必须先生成完整的逐项文档变更清单。清单覆盖根目录文档、业务层文档、基础设施层文档及拟创建文档；仅「新建」和经开发者确认的「修改」项可以写入。
...
| 文档 | 操作 | 理由 |
|------|------|------|
| `README.md` | 不动 | 已存在，未获修改确认。 |
| `INDEX.md` | 新建 | 项目缺少导航，开发者要求建立文档。 |
| `src/model/business/README.md` | 修改 | [仅在开发者确认后列出具体理由] |

请确认清单中标为「新建」和「修改」的每一项；「不动」项不会被写入。
```

若子代理的变更涉及项目级文档需要同步更新，协调者还必须生成以下项目级变更建议：
